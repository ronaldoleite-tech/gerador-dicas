...
let carregandoUltimosGeral = false; // Flag para evitar múltiplas requisições
...
// ==============================
const lotteryConfig = {
    maismilionaria: { nome: "+Milionária", min_dezenas: 6, max_dezenas: 12, num_bolas: 6, universo: 50, default_dezenas: 6, num_trevos: 2, universo_trevos: 6 }, // Adicionado
    ...
    timemania:  { nome: "Timemania",   min_dezenas: 10, max_dezenas: 10, num_bolas: 7,  universo: 80,  default_dezenas: 10 }, // Adicionado
    supersete:  { nome: "Super Sete",  min_dezenas: 7,  max_dezenas: 7,  num_bolas: 7,  universo: 9,   default_dezenas: 7 } // Adicionado
};
...
//  ÚLTIMOS RESULTADOS GERAL
async function carregarUltimosResultadosGeral() {
    if (carregandoUltimosGeral) return;

    carregandoUltimosGeral = true;
    const containerGeral = getElement("ultimos-resultados-geral");
    const spinnerGeral = getElement("spinner-ultimos-geral");

    if (!containerGeral || !spinnerGeral) {
        carregandoUltimosGeral = false;
        return;
    }

    spinnerGeral.style.display = 'block'; // Mostrar spinner
...
// ...
    ...
    try {
        const promises = loteriasParaExibir.map(async (loteriaKey) => {
            const config = lotteryConfig[loteriaKey];
            const response = await fetch(`/get-ultimos-resultados?loteria=${loteriaKey}&limite=1`); // Limite de 1 resultado
            
            if (!response.ok) {
                console.error(`Falha ao carregar o último resultado da ${config.nome}.`);
                return ''; // Retornar string vazia para não quebrar tudo
            }
            
            const resultados = await response.json();
            
            if (resultados.error || resultados.length === 0) {
                console.warn(`Nenhum resultado encontrado para a ${config.nome} ou erro: ${resultados.error}`);
                return '';
            }
            ...
            return `
                <div class="ultimo-resultado-card">
                    <h4 class="nome-loteria-card">${config.nome}</h4>
                    ${criarHtmlResultado(ultimoResultado, loteriaKey)}
                </div>
            `;
        });

        const resultsHtmlArray = await Promise.all(promises);
        allResultsHtml = resultsHtmlArray.join('');
        ...
        if (allResultsHtml) {
            ...
            containerGeral.innerHTML = `
                <h3 style="color: var(--cor-dourado); text-align: center; margin-bottom: 1rem;">Resultados das Loterias</h3>
                <p style="text-align: center; color: #ff8a80;">Não foi possível carregar os últimos resultados de algumas loterias.</p>
            `;
        }

    } catch (error) {
        console.error("Erro ao carregar últimos resultados gerais:", error);
        containerGeral.innerHTML = `
        ...
        spinnerGeral.style.display = 'none'; // Esconder spinner
        carregandoUltimosGeral = false;
    }
}
...
function criarHtmlResultado(resultado, loteria) {
    ...
    // Lógica específica para Mais Milionária
    if (loteria === "maismilionaria" && resultado.dezenas && resultado.trevos) {
        const dezenasArray = resultado.dezenas.split(' ');
        const trevosArray = resultado.trevos.split(' ');

        const dezenasFormatadas = dezenasArray.map(num => String(num).padStart(2, '0')).join(' ');

        dezenasHtml = `
            <div class="resultado-dezenas">${dezenasFormatadas}</div>
            <div style="margin-top: 0.5rem; font-size: 1.1rem;">
                <span style="color: var(--cor-texto);">Trevos: </span>
                <span style="color: var(--cor-dourado); font-weight: 700;">${trevosArray.join(' ')}</span>
            </div>
        `;
    }
    // Lógica específica para Dupla Sena
    else if (loteria === "duplasena" && resultado.dezenas) {
        const dezenasArray = resultado.dezenas.split(' ');
        if (dezenasArray.length === 12) {
            const primeiroSorteio = dezenasArray.slice(0, 6);
            const segundoSorteio = dezenasArray.slice(6, 12);
            dezenasHtml = `
    ...
    // Lógica específica para Timemania
    else if (loteria === "timemania" && resultado.dezenas && resultado.time_coracao) {
        const dezenasArray = resultado.dezenas.split(' ');
        const dezenasFormatadas = dezenasArray.map(num => String(num).padStart(2, '0')).join(' ');
        ...
        dezenasHtml = `
            <div class="resultado-dezenas">${dezenasFormatadas}</div>
            <div style="margin-top: 0.5rem; font-size: 1.1rem;">
                <span style="color: var(--cor-texto);">Time do Coração: </span>
                <span style="color: var(--cor-dourado); font-weight: 700;">${resultado.time_coracao}</span>
            </div>
        `;
    }
    // Lógica específica para Super Sete
    else if (loteria === "supersete" && resultado.dezenas) {
        const dezenasArray = resultado.dezenas.split('');
        dezenasHtml = `
            <div style="display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; margin: 10px 0;">
                ${dezenasArray.map((dezena, index) => `
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 4px;">
                        <span style="font-size: 0.9rem; color: var(--cor-dourado); opacity: 0.8;">${index + 1}</span>
                        <span style="font-size: 1.7rem; font-weight: 700; color: var(--cor-dourado);">${dezena}</span>
                    </div>
                `).join('')}
            </div>
        `;
    } else {
        // Formatação padrão para outras loterias e fallback
        const dezenasArray = resultado.dezenas ? resultado.dezenas.split(' ') : [];
        const dezenasFormatadas = dezenasArray.map(num => String(num).padStart(2, '0')).join(' ');
        dezenasHtml = `<div class="resultado-dezenas">${dezenasFormatadas}</div>`;
    }
...
function criarStatusHtml(resultado) {
    ...
    if (resultado.valor_acumulado) {
        ...
        html += `<div class="valor-acumulado">Acumulada próximo concurso: R$ ${valorFormatado}</div>`;
    }
...
function formatarData(data) {
    ...
    try {
        ...
        if (dia && mes && ano && !isNaN(new Date(ano, mes - 1, dia))) {
            const d = new Date(ano, mes - 1, dia);
            return `${String(d.getDate()).padStart(2, '0')}/${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
        }
    } catch (e) {
        console.warn("Erro ao formatar data, tentando formato alternativo:", data, e);
    }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    carregarUltimosResultadosGeral(); 
});
